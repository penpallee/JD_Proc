...
namespace JD_Proc.ImageFilter
{
    public class GaussianBlur
    {
        ...
        public static List<List<double>> ApplyGaussianBlurForCSV(List<List<double>> data, int height, int width, string resultcsv)
        {
            // 3x3 Gaussian kernel with σ = 1.0 (approximation)
            ...
            var result = new List<List<double>>(data.Count);

            // 데이터 복사 및 0으로 초기화
            for (int i = 0; i < data.Count; i++)
            {
                result.Add(new List<double>(new double[data[0].Count]));
            }
            ...
            using (StreamWriter sw = new StreamWriter(resultcsv))
            {
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        string csvLine = Math.Round(result[y][x], 1).ToString();
                        if (y == height) sw.Write(csvLine);
        ...
        public void ConvertBmpToCsv(string bmpFilePath, string csvFilePath)
        {
